...
def run_analysis(job_id, data):
    try:
        ...
        **[추가 검토 체크리스트 (지역지구 및 목적 기반)]**
        ...
        1. **안전 검토 (매우 중요)**: [사업 개요]의 주요 사업 내용을 분석하여, '건설기술 진흥법' 등에 따른 **설계안전성검토** 및 **안전관리계획서**(또는 유해위험방지계획서) 제출 대상 여부를 추론하십시오. (예: 10m 이상 깊이 굴착, 10층 이상 건축물 등 위험 요소가 있는지 보수적으로 판단)
        2. **매장유산 보호 및 조사에 관한 법률**: 문화재 지표조사 대상 여부 파악
        3. **도시교통정비 촉진법**: 교통영향평가 대상 여부
        4. **국토의 계획 및 이용에 관한 법률**: 개발행위허가 및 도시계획위원회 심의 대상 여부
        5. **산지관리법 / 농지법**: 산지전용허가, 농지전용허가 대상 여부 (산지/농지 포함 시)
