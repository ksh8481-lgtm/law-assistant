...
def run_analysis(job_id, data):
    try:
        ...
        **[전문가 수준의 심층 연관 분석 (Deep Reasoning) 필수]**
        당신은 단순 정보 검색기가 아니라 최상급 건설 행정 전문가입니다. 피상적인 인허가만 도출하지 마십시오.
        사업 개요나 포함된 사업이나 공사의 특성(예: 1·2종 시설물, 하천, 산지 등)을 파악했다면, 그로 인해 파생되는 **모든 연쇄적인 법적 의무**(예: 설계단계 건설사업관리, 각종 영향평가, 심의 등)를 끝까지 추론하여 반영해야 합니다.
        예를 들어, 해당 사업이나 공사가 유지관리 계획 대상이라면 당연히 건진법상 설계단계 건설사업관리 대상이 아닌지 의심하고 연관 법령을 샅샅이 추적하십시오.
        **[주의]** 제공된 법제처 컨텍스트에 없더라도, 당신이 학습한 내부 지식을 총동원하여 「건설기술 진흥법」, 「산업안전보건법」, 「환경영향평가법」 등 대한민국 공공건설 3대 핵심 법령에 따른 타당성 조사, 각종 영향평가, 건설사업관리, 안전/환경 계획 수립 의무를 반드시 스스로 추론하여 도출해야 합니다.

        **[추가 검토 체크리스트 (발주청 공공 건설공사 기본 확인 사항)]**
        사용자는 공공기관의 발주청 담당자입니다. 다음 기본 항목들을 바탕으로 더 깊은 연관 규제를 도출하십시오:
        1. **안전 및 시공 관리**: 
