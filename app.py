...
3. 🔎 **판례/해석례 절대 창작 금지 (환각 완전 차단)**: 대법원 판례나 유권해석은 **오직 [법제처 API 실시간 RAG 검색 결과]에 제공된 목록 안에서만** 골라서 인용하십시오. 구글 검색이나 내부 지식을 동원하여 외부 판례를 억지로 끌어오지 마십시오. **만약 RAG 검색 결과에 판례나 유권해석이 0개라면, 절대 지어내지 말고 쿨하게 "검색된 관련 판례/유권해석 없음"이라고만 적으십시오.**
