...
def fetch_local_law_data(query, moleg_context):
    ...
    # 조사가 붙은 원형("재난관리기금으로")은 쓰지 않고, 조사를 뗀 형태("재난관리기금")만
    # 쓴다. 원형이 실제로 원문에 등장하는 자리는 항상 그 조사 뗀 형태가 등장하는
    # 자리와 같거나 그 부분집합이라(원형 = 뗀형태+조사), 원형을 keyword로 같이
    # 넣어봤자 얻는 정보가 없고, 오히려 사고를 냈다: "재난관리기금을"(원형)이
    # "재난관리기금"보다 길다는 이유로 먼저 처리되면서, 하필 재난관리기금이 몰려
    # 있는(=진짜 관련된) 그 자리 한가운데를 미리 차지해버려 "재난관리기금" 자신이
    # 정작 그 자리를 못 쓰고 무관한 위치로 밀려나는 사고로 이어졌음(실사용 중 발견).
    locate_keywords = set(_strip_korean_particle(w) for w in (keywords | raw_locate_words))
...
def _extract_relevant_excerpt(content, keywords, max_len):
    ...
    # 같은 키워드의 여러 등장 위치를 "인접한(1200자 이내) 것끼리" 클러스터로
    ...
    #
    # 처리 순서도 "등장 횟수가 적은 것부터"만으로는 부족하다는 게 실사용 중 추가로
    # 드러났다: "근거"/"가능"/"여부"처럼 사용자의 일반 서술어투에서 나온 2~3자
    # 짧은 단어가, 하필 법 원문에는 어쩌다 1~5번밖에 안 나와서 "희귀한 키워드"로
    # 착각되어 "재난관리기금"(6자, 9번)보다 먼저 처리되고 예산을 다 가져가 버렸다.
...
6. 🚨 **[하위 항목 개수 조작 절대 금지]**: 어떤 조항이 [로컬 법령 데이터베이스]에 원문 그대로 제공되어 있다면, 그 조항의 호(1. 2. 3. ...)ㆍ목(가. 나. 다. ...) 구성을 **절대로 실제보다 더 많이 지어내지 마십시오.** 인용하기 전에 제공된 원문에서 그 조항에 실제로 몇 개의 호가 있는지 직접 세어보고, 그 범위를 벗어나는 호(예: 원문엔 제1호ㆍ제2호뿐인데 제3호 이상을 언급)를 절대 만들어내지 마십시오. 원문에 있는 조항은 반드시 원문의 문구를 그대로 옮기고, 원문에 없는 세부 항목이 필요하다고 판단되면 지어내는 대신 "구체적인 하위 항목은 원문을 직접 확인하십시오"라고 안내하십시오.
