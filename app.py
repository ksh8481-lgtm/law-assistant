...
def run_analysis(job_id, data):
    try:
        ...
        **[필수 종합 검토 체크리스트 (절대 누락 금지)]**
        다음 항목들은 지역지구와 무관하게 사업의 **면적({total_area}㎡) 및 목적**을 기준으로 무조건 대상 여부를 판단하여 보고서에 반영하십시오:
        1. **자연재해대책법**: 재해영향평가 (개발사업 면적 기준 검토)
        2. **환경영향평가법**: 환경영향평가 또는 소규모 환경영향평가 대상 여부
        3. **매장유산 보호 및 조사에 관한 법률**: 문화재 지표조사 대상 여부 (통상 3만㎡ 이상)
        4. **도시교통정비 촉진법**: 교통영향평가 대상 여부
        5. **국토의 계획 및 이용에 관한 법률**: 개발행위허가 및 도시계획위원회 심의 대상 여부
        6. **산지관리법 / 농지법**: 산지전용허가, 농지전용허가 대상 여부 (산지/농지 포함 시)
