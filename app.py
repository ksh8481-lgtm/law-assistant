...
def run_analysis(job_id, data):
    try:
        ...
        **[전문가 수준의 심층 연관 분석 (Deep Reasoning) 필수]**
        ...
        사업 개요나 포함된 시설물의 특성(예: 1·2종 시설물, 하천, 산지 등)을 파악했다면, 그로 인해 파생되는 **모든 연쇄적인 법적 의무**(예: 설계단계 건설사업관리, 각종 영향평가, 심의 등)를 끝까지 추론하여 반영해야 합니다.
        예를 들어, 시설물 유지관리 계획 대상이라면 당연히 건진법상 설계단계 건설사업관리 대상이 아닌지 의심하고 연관 법령을 샅샅이 추적하십시오.
