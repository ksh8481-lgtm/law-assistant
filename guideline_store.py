...
API가 없어 자동으로 가져올 수 없다. 이후 모든 "설계도서 표준·지침 검토"에서
자동으로 참고 자료로 활용하기 위해, 문서를 여기 상시 보관해둔다.

⚠️ 등록/삭제는 사이트(웹 UI)에 노출하지 않는다(불특정 다수가 접근 가능한
배포 사이트에 인증 없는 업로드/삭제 창구를 열어두는 것 자체가 바람직하지
않기도 함). 새 지침 추가나 개정판 교체는 개발자가 Claude Code 세션에서 이
모듈의 add_guideline()/delete_guideline()을 직접 호출해서 처리하고, 그 결과
(data/guidelines/)를 git commit/push로 배포한다(Cloudtype은 재배포 시
디스크가 초기화되므로 로컬 작업이 필수). 사이트에는 list_guidelines() 결과만
조회 전용으로 노출해서(/api/guidelines GET), 지금 무엇이 검토에 반영되고
있는지 확인만 할 수 있게 한다.
"""
import os
import json
